...
class Aplicarccp_model extends CI_Model {						// **->

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public function __construct() {
	$this->config->load('nocrud_aplicar_ccp.php');						// **->
	$this->load->database();
	...
	if ($idEnti > 0){
		...
		AND (vcc.tc_tipos_entidad->>'tipos_entidad'::text)::jsonb ? '2'
		AND vcc.cc_estado = 0
...
//		AND vcc.cc_proyecto_id = $idProy
		...
		$sql = "SELECT    cc_id
			...
			FROM vi_ctas_ctes vcc
			WHERE vcc.cc_entidad_id = $idEnti
			AND vcc.tc_modelo NOT IN (1,2,4)
			AND vcc.cc_estado = 0
...
//		AND estado = 0
	...
	$sql1 = "UPDATE relacion_ctas_ctes SET estado = 1 
		WHERE cuenta_corriente_id IN (SELECT cc.id AS id 
					    ...
					    WHERE cc.entidad_id = ? 
					      AND tc.tipos_entidad->>'tipos_entidad'::text LIKE '%2%' )";      /// para provee tienen q ser todos los proy   AND proyecto_id = ?)";
