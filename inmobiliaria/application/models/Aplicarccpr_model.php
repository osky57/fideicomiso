...
class Aplicarccpr_model extends CI_Model {						// **->

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public function __construct() {
	$this->config->load('nocrud_aplicar_ccpr.php');						// **->
	$this->load->database();
	...
	if ($idEnti > 0){
	    $sql = "SELECT  vcc.cc_id AS vcc_cc_id,
			    ...
			    fun_calcimportecomprob_1(vcc.cc_id,1,0)  AS importe_mn,
			    fun_calcimportecomprob_1(vcc.cc_id,2,0)  AS importe_div,
		...
		AND vcc.tc_modelo IN (4)
		AND (vcc.tc_tipos_entidad->>'tipos_entidad'::text)::jsonb ? '3'
		AND vcc.cc_estado = 0
		AND vcc.cc_fecha BETWEEN '$desde' AND '$hasta'
		ORDER BY vcc.cc_fecha DESC, vcc.cc_id DESC";

//tiene q traer recibos (modelo 4) para entidad prestamista (tipo entidad 3)
...
//		AND estado = 0
	...
	//anula todos los mov. de la entidad en el proyecto activo
	$sql1 = "UPDATE relacion_ctas_ctes SET estado = 1 
		WHERE cuenta_corriente_id IN (SELECT cc.id AS id 
					    ...
					    WHERE cc.entidad_id = ? 
					      AND tc.tipos_entidad->>'tipos_entidad'::text LIKE '%3%' )";      /// para provee tienen q ser todos los proy   AND proyecto_id = ?)";

wh_log("enti $idEntidad");
wh_log($sql1);
