...
class Entidades extends CI_Controller {								// **->
	...
	public function cargaFormulario(){
		...
		$data  = $enti->devuelveUnRegistro($elId);
		$data  = $data[0];
		$data['urlvalcuit'] = $this->config->item('urlValCuit');
		$vista = $_GET['name'];
		$this->load->view($vista,$data);
	}

	//////////////////////////////////////////////////////////////////////////////////////////
	public function validaCuit(){

	    $elId  = $_GET['identi'];
	    $cuit  = $_GET['cuit'];

	    $aArr = array('elId' => $_GET['identi'],
			  'cuit' => $_GET['cuit']);

	    $enti  = new Entidades_model();							// **->
	    $data  = $enti->validaCuit($elId,$cuit);
...
wh_log("111111111111111111111111111111111111");
